...
//#define TEST_NUM (1000)              // 测试最大组数
#define CASCADE_FLAG (1)               // Bob:1 , Alice:0 
...
{
	// 0. 初始化
	initServerConn(serverSocket, conn);
	...
	fp_data = fopen("../../Data/sifted_key_bob", "rb");
	if (fp_data == NULL){
		printf("文件读取失败!\n");
		exit(1);
	}
...
inline void initData(unsigned char* posData, int dataSize, double qber)
{
	...
	//int dataRndMax = (int)(RAND_MAX * qber);

	//for (int i = 0; i < dataSize; i++) {
	//	unsigned char dataByte = disInfo(rndInfo);

	//	for (int j = 0; j < 8; j++) {  // Bob端加入误码, Alice端保持不变
	//		if (disQBER(rndQBER) < dataRndMax)
	//			dataByte = dataByte ^ (FLAG_BOB << j);
	//	}
...
inline void statOutput(int t)
{
	...
	for (int index = 0; index < OMP_THREADS_NUM; index++) {
		...
		printf("数据帧数: %d, 总交互次数: %d, 协商效率: %.3f\n\n", inputStat[index] / (64*1024), interactStat[0], f);
	}
}
